import type { VehicleConfig, VehicleState } from './types';
...
export class VehicleStateSimulator {
  ...
  private selectGear(): 'N' | '1' | '2' | '3' | '4' | '5' {
  ...
  private applyDoorRandom() {
    if (Math.random() < 0.001 && this.state.speed < 0.5) {
      const doors = ['frontLeft', 'frontRight', 'rearLeft', 'rearRight'] as const;
      const door = doors[Math.floor(Math.random() * doors.length)];
      this.state.doorStatus[door] = !this.state.doorStatus[door];
    }
  }

  private applyFaultRandom() {
    if (Math.random() < 0.0003 && this.state.faultCodes.length === 0) {
      this.state.faultCodes = [[101], [300], [420]][Math.floor(Math.random() * 3)];
    }
    if (Math.random() < 0.008 && this.state.faultCodes.length > 0) {
      this.state.faultCodes = [];
    }
  }
  ...
  tick(): VehicleState {
    if (this.driving) {
      ...
      this.state.coolantTemp = Math.min(85, this.state.coolantTemp + 0.15);
      this.state.coolantTemp += Math.random() * 0.3 - 0.15;
      ...
      const fuelBurn =
        this.state.rpm > 800
          ...
          : 0;
      this.state.fuelLevel = Math.max(0, this.state.fuelLevel - fuelBurn);

      this.state.gearPosition = this.selectGear();

      if (this.state.speed < 0.5 && this.state.gearPosition === 'N' && Math.random() < 0.002) {
        this.state.doorStatus.frontLeft = !this.state.doorStatus.frontLeft;
      }

      this.applyDoorRandom();
      this.applyFaultRandom();
      ...
      if (this.state.speed > 0.5) this.state.brakePressed = true;
      else this.state.brakePressed = false;
